...
void param_sfo::param_sfo_file::read_index_table_entry(std::ifstream& in_stream, const uint& i, index_table_entry_t& index_table_entry) {
    byte buffer[4];
...
void param_sfo::param_sfo_file::read_param_entry(std::ifstream& in_stream, index_table_entry_t& index_table_entry, param_entry_t& param_entry) {
    ...
    while(in_stream.peek() != '\0')
        param_entry.keys_table_entry += in_stream.get();
    ...
    for(uint i = 0; i < param_entry.datum_len; i++)
        param_entry.data_table_entry += in_stream.get();
...
uint param_sfo::param_sfo_file::to_uint(byte LE_array[], const int& size) {
    uint num = 0;

    for(int i = size - 1; i >= 0; i--)
        num = (num << 8) + LE_array[i];

    return num;
}

//constructor
param_sfo::param_sfo_file::param_sfo_file(const std::string& path) {
    this->path = path;
...
void param_sfo::param_sfo_file::print(const bool& hex) {
    std::cout << "PATH\n" << this->path << "\n\n";
    std::cout << "SFO VERSION\n";

    if(!hex)
        std::cout << this->header.version << "\n\n";
    else
        std::cout << std::hex << std::setfill('0') << std::setw(4) << std::uppercase << this->header.version << "\n\n";

    for(const param_entry_t& param_entry : this->param_table.entries) {
        std::cout << param_entry.keys_table_entry + " - Fmt: " + param_entry.datum_fmt << "\n";

        if(!hex)
            std::cout << param_entry.data_table_entry << "\n\n";
        else {
            for(std::size_t i = 0; i < param_entry.data_table_entry.size(); i++)
                std::cout << std::hex << std::setfill('0') << std::setw(2) << std::uppercase << static_cast<uint>(param_entry.data_table_entry[i]);
